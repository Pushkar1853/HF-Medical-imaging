...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        self.num_points_embeddings: int = 4
        point_embeddings = [nn.Embedding(1, embed_dim)
                            for i in range(self.num_points_embeddings)]
        self.points_embeddings = nn.ModuleList(point_embeddings)
    ...
    def _embed_points(
        ...
        if pad:
            ...
            points = torch.cat([labels, padding_label], dim=1)
    ...
    def _get_device(self) -> torch.device:
        return self.points_embeddings[0].weight.device
    ...
    def forward(
        ...
        if boxes is not None:
            ...
            sparse_embeddings = torch.cat(
                [sparse_embeddings, box_embedding], dim=1)
