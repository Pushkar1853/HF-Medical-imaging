    ...
    def cat(self, new_stats: "MaskData") -> None:
        for k, v in new_stats.items():
            if k not in self._stats or self._stats[k] is None:
                ...
                raise TypeError(
                    f"MaskData key {k} has an unsopported type {type(v)}.")
...
def batch_iterator(batch_size: int, *args) -> Generator[List[Any], None, None]:
    ...
    n_batches = len(args[0]) // batch_size + \
        int(len(args[0]) % batch_size != 0)
    for b in range(n_batches):
        yield [args[b * batch_size: (b+1) * batch_size] for arg in args]
...
def build_all_layer_points_grids(
...
def generate_crop_boxes(
    ...
    crop_boxes.append([0, 0, im_h, im_w])
    layer_idxs.append(0)
    ...
    for i_layer in range(n_layers):
        ...
        crop_box_x0 = [int((crop_w - overlap) * i)
                       for i in range(n_crops_per_side)]
        crop_box_y0 = [int((crop_h - overlap) * i)
                       for i in range(n_crops_per_side)]
